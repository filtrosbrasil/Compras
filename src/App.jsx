...
import { ETAPAS, CATEGORIAS, STATUS_EXTRA, EVENTOS, RESPONSAVEIS, INITIAL_DATA } from './data.js';
...
function Badge({ label, color, bg }) {
  return <span style={{ display:"inline-block",padding:"2px 7px",borderRadius:4,fontSize:10,fontWeight:700,color,background:bg,border:`1px solid ${color}22`,whiteSpace:"nowrap" }}>{label}</span>;
}
...
function PipelineStep({ done, current, label, onClick }) {
  ...
  return (
    <div onClick={onClick} title={label} style={{ width:26,height:26,borderRadius:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,cursor:"pointer",border:`1.5px solid ${border}`,background:bg,color,transition:"transform 0.15s",flexShrink:0 }}
...
function ResponsavelInline({ value, onChange }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",fontSize:11,fontWeight:600,color:"#0B1F3F",cursor:"pointer",fontFamily:"inherit",maxWidth:90,appearance:"auto" }}>
...
function ModalForm({ data, onSave, onClose }) {
  ...
  const labelStyle = { display:"block",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:1,color:"#5A6A85",marginBottom:4 };
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(11,31,63,0.4)",backdropFilter:"blur(6px)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"white",borderRadius:10,padding:isMobile?20:28,width:isMobile?"100%":520,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 8px 32px rgba(11,31,63,0.12)" }}>
        <h2 style={{ fontSize:18,fontWeight:800,marginBottom:16,color:"#0B1F3F" }}>{form.id ? "✏️ Editar" : "＋ Nova"} <span style={{ color:"#E31E24" }}>Demanda</span></h2>
        ...
        <div style={{ display:"flex",gap:8,justifyContent:"flex-end",paddingTop:14,borderTop:"1px solid #E2E6EE" }}>
          <button onClick={onClose} style={{ padding:"9px 18px",borderRadius:6,border:"1px solid #E2E6EE",background:"white",color:"#0B1F3F",fontFamily:"inherit",fontSize:13,fontWeight:600,cursor:"pointer" }}>Cancelar</button>
...
export default function App() {
  const { data: demandas, save, loaded, reset } = useStorage(INITIAL_DATA);
  ...
  const saveModal = (form) => {
    ...
    if (form.id) { save(demandas.map(d => d.id === form.id ? { ...d, ...form } : d)); flash("Atualizada"); }
    else { save([...demandas, { ...form, id: Date.now().toString(), etapas:{orcamento:false,aprovacao:false,contrato:false,contrato_assinado:false,pedido_colocado:false} }]); flash("Adicionada"); }
    setModal(null);
  };

  const excluir = (id) => { if (confirm("Excluir?")) { save(demandas.filter(d => d.id !== id)); flash("Excluída"); } };

  if (!loaded || !demandas) return <div style={{ display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F2F4F7",fontFamily:"Amplesoft,Nunito,sans-serif",fontSize:16 }}>Carregando...</div>;
    ...
    return (
      <div key={d.id} style={{ background:"white",borderRadius:10,padding:14,border:`1px solid ${isOverdue?"rgba(227,30,36,0.2)":"#E2E6EE"}`,boxShadow:"0 1px 4px rgba(11,31,63,0.06)",borderLeft:isOverdue?"3px solid #E31E24":"3px solid transparent" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
          <div>
            <div style={{ fontWeight:700,fontSize:13,marginBottom:4 }}>{d.item}</div>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:4,marginBottom:8 }}>
          ...
          {d.prazo && <span style={{ fontSize:10,fontWeight:700,marginLeft:"auto",color:isOverdue?"#E31E24":isSoon?"#E07800":"#5A6A85" }}>{isOverdue?"⚠ ":""}{fmtData(d.prazo)}{isOverdue?` (${Math.abs(dias)}d)`:isSoon?` (${dias}d)`:""}</span>}
        </div>
        <div style={{ display:"flex",gap:3,alignItems:"center" }}>
          {ETAPAS.map((e, ei) => <PipelineStep key={e.key} done={d.etapas[e.key]} current={ei===(ea===-1?ETAPAS.length:ea)} label={e.short} onClick={() => toggleEtapa(d.id, e.key)} />)}
          <span style={{ fontSize:11,fontWeight:700,color:pct===100?"#0FAA6E":"#5A6A85",marginLeft:6 }}>{pct}%</span>
        </div>
  ...
  const renderRow = (d) => {
    ...
    return (
      <tr key={d.id} style={{ background:isOverdue?"rgba(227,30,36,0.03)":"transparent",borderBottom:"1px solid #E2E6EE" }}>
        <td style={{ padding:10,maxWidth:200 }}><strong style={{ fontSize:12 }}>{d.item}</strong>{d.notas && <span title={d.notas} style={{ marginLeft:3,opacity:0.5,fontSize:11,cursor:"help" }}>💬</span>}</td>
        ...
        <td style={{ fontSize:11,fontWeight:isDone?400:600,color:isDone?"#0FAA6E":isOverdue?"#E31E24":isSoon?"#E07800":"#5A6A85",whiteSpace:"nowrap" }}>{d.prazo?`${isOverdue?"⚠ ":""}${fmtData(d.prazo)}${isOverdue?` (${Math.abs(dias)}d)`:""}` : "—"}</td>
        <td>{se?.value ? <Badge label={`${se.icon} ${se.label}`} color={se.color} bg={se.bg} /> : "—"}</td>
        <td><div style={{ display:"flex",gap:2 }}>{ETAPAS.map((e, ei) => <PipelineStep key={e.key} done={d.etapas[e.key]} current={ei===(ea===-1?ETAPAS.length:ea)} label={e.short} onClick={() => toggleEtapa(d.id, e.key)} />)}</div></td>
        <td><div style={{ display:"flex",alignItems:"center",gap:4 }}><div style={{ width:40,height:4,background:"#E2E6EE",borderRadius:2,overflow:"hidden" }}><div style={{ width:`${pct}%`,height:"100%",background:pc,borderRadius:2 }} /></div><span style={{ fontSize:10,color:"#5A6A85",fontWeight:600 }}>{pct}%</span></div></td>
        <td><div style={{ display:"flex",gap:2 }}><button onClick={() => setModal(d)} style={{ width:26,height:26,borderRadius:5,border:"none",background:"transparent",cursor:"pointer",fontSize:11 }}>✏️</button><button onClick={() => excluir(d.id)} style={{ width:26,height:26,borderRadius:5,border:"none",background:"transparent",cursor:"pointer",fontSize:11 }}>🗑</button></div></td>
      </tr>
    );
  };
    ...
    <div style={{ fontFamily:"Amplesoft,Nunito,Segoe UI,sans-serif",background:"#F2F4F7",minHeight:"100vh",color:"#0B1F3F" }}>
      <div style={{ background:"#0B1F3F",padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:56,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 12px rgba(11,31,63,0.25)" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          ...
          <div>
            <div style={{ color:"white",fontSize:15,fontWeight:700 }}>Controle de <span style={{ color:"#FF4D52" }}>Compras</span></div>
        ...
        <div style={{ display:"flex",gap:6 }}>
          {!isMobile && <button onClick={() => setView(v => v==="table"?"cards":"table")} style={{ padding:"7px 14px",borderRadius:6,border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.85)",fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer" }}>{view==="table"?"📱 Cards":"📊 Tabela"}</button>}
          <button onClick={() => setModal({item:"",categoria:"brindes",evento:"convencao",fornecedor:"",responsavel:"Maria Eduarda",urgencia:"media",valor:"",notas:"",status_extra:"",prazo:""})} style={{ padding:"7px 14px",borderRadius:6,border:"none",background:"#E31E24",color:"white",fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer" }}>＋ Nova</button>
        </div>
      </div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:isMobile?"repeat(3,1fr)":"repeat(5,1fr)",gap:8,padding:"12px 16px" }}>
        {[{n:stats.total,l:"Total",c:"#0B1F3F"},{n:stats.concluidos,l:"Concluídos",c:"#0FAA6E"},{n:stats.andamento,l:"Andamento",c:"#E5A100"},{n:stats.pendentes,l:"Pendentes",c:"#E31E24"},{n:stats.atrasados,l:"⚠ Atrasados",c:"#E31E24"}].map((s,i) => (
          <div key={i} style={{ background:"white",borderRadius:8,padding:"10px 12px",textAlign:"center",border:"1px solid #E2E6EE" }}>
            <div style={{ fontSize:22,fontWeight:800,color:s.c }}>{s.n}</div>
            <div style={{ fontSize:8,color:"#5A6A85",textTransform:"uppercase",letterSpacing:1,fontWeight:600,marginTop:2 }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ padding:"0 16px 8px",display:"flex",gap:6,flexWrap:"wrap" }}>
        {STATUS_EXTRA.filter(s=>s.value).map(s => (
          <div key={s.value} onClick={() => setFilters(f=>({...f,sit:f.sit===s.value?"todos":s.value}))} style={{ display:"flex",alignItems:"center",gap:5,padding:"5px 10px",borderRadius:16,background:filters.sit===s.value?"rgba(11,31,63,0.06)":"white",border:`1px solid ${filters.sit===s.value?"#0B1F3F":"#E2E6EE"}`,fontSize:10,fontWeight:600,color:filters.sit===s.value?"#0B1F3F":"#5A6A85",cursor:"pointer",whiteSpace:"nowrap" }}>
            {s.icon} {s.label.split("(")[0].trim()}
            <span style={{ background:s.bg,color:s.color,padding:"1px 6px",borderRadius:8,fontSize:9,fontWeight:800 }}>{sitCounts[s.value]||0}</span>
          </div>
        ))}
        {filters.sit!=="todos" && <div onClick={() => setFilters(f=>({...f,sit:"todos"}))} style={{ padding:"5px 10px",borderRadius:16,border:"1px solid rgba(227,30,36,0.2)",fontSize:10,fontWeight:600,color:"#E31E24",cursor:"pointer" }}>✕</div>}
      </div>

      {/* Filters */}
      <div style={{ padding:"0 16px 12px",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center" }}>
        <div style={{ display:"flex",gap:3,background:"white",padding:"4px 5px 4px 10px",borderRadius:20,border:"1px solid #E2E6EE",alignItems:"center",flexWrap:"wrap" }}>
          <span style={{ fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:1.2,color:"#94A0B8",paddingRight:6,borderRight:"1px solid #E2E6EE",marginRight:3 }}>Tipo</span>
          {[{value:"todos",label:"Todos"},...CATEGORIAS].map(c => (
            <button key={c.value} onClick={() => setFilters(f=>({...f,cat:c.value}))} style={{ padding:"4px 8px",borderRadius:14,border:"none",background:filters.cat===c.value?"#0B1F3F":"transparent",color:filters.cat===c.value?"white":"#5A6A85",fontFamily:"inherit",fontSize:10,fontWeight:600,cursor:"pointer" }}>{c.label}</button>
          ))}
        </div>
        <div style={{ display:"flex",gap:3,background:"white",padding:"4px 5px 4px 10px",borderRadius:20,border:"1px solid #E2E6EE",alignItems:"center" }}>
          <span style={{ fontSize:8,fontWeight:700,textTransform:"uppercase",letterSpacing:1.2,color:"#94A0B8",paddingRight:6,borderRight:"1px solid #E2E6EE",marginRight:3 }}>Resp.</span>
          <button onClick={() => setFilters(f=>({...f,resp:"todos"}))} style={{ padding:"4px 8px",borderRadius:14,border:"none",background:filters.resp==="todos"?"#0B1F3F":"transparent",color:filters.resp==="todos"?"white":"#5A6A85",fontFamily:"inherit",fontSize:10,fontWeight:600,cursor:"pointer" }}>Todas</button>
          {RESPONSAVEIS.map(r => <button key={r} onClick={() => setFilters(f=>({...f,resp:r}))} style={{ padding:"4px 8px",borderRadius:14,border:"none",background:filters.resp===r?"#E31E24":"transparent",color:filters.resp===r?"white":"#5A6A85",fontFamily:"inherit",fontSize:10,fontWeight:600,cursor:"pointer" }}>{r.split(" ")[0]}</button>)}
        </div>
        <div style={{ flex:1 }} />
        <input placeholder="🔍 Buscar..." value={filters.q} onChange={e => setFilters(f=>({...f,q:e.target.value}))} style={{ padding:"7px 14px",borderRadius:16,border:"1px solid #E2E6EE",background:"white",color:"#0B1F3F",fontFamily:"inherit",fontSize:12,width:isMobile?120:200,outline:"none" }} />
      </div>

      {/* Content */}
      <div style={{ padding:"0 16px 32px" }}>
        {filtered.length === 0 ? <div style={{ textAlign:"center",padding:60,color:"#94A0B8" }}>📦 Nenhuma demanda</div>
        : (isMobile || view === "cards") ? <div style={{ display:"flex",flexDirection:"column",gap:8 }}>{filtered.map(renderCard)}</div>
        : <div style={{ overflowX:"auto",borderRadius:10,border:"1px solid #E2E6EE",boxShadow:"0 4px 16px rgba(11,31,63,0.08)" }}>
            <table style={{ width:"100%",borderCollapse:"collapse",background:"white",fontSize:12 }}>
              <thead><tr style={{ background:"#F5F6FA" }}>
                {["Item","Tipo","Evento","Resp.","Urg.","Prazo","Situação","Pipeline","%",""].map((h,i) => <th key={i} style={{ padding:"10px",textAlign:"left",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:1.2,color:"#94A0B8",borderBottom:"2px solid #E2E6EE",whiteSpace:"nowrap" }}>{h}</th>)}
              </tr></thead>
      ...
      {toast && <div style={{ position:"fixed",bottom:20,right:20,background:"#0B1F3F",color:"white",padding:"10px 18px",borderRadius:8,fontSize:13,fontWeight:600,boxShadow:"0 8px 32px rgba(11,31,63,0.2)",zIndex:2000 }}>✓ {toast}</div>}
    </div>
  );
}
