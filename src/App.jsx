...
const PStep = ({ done, current, label, onClick }) => {
  ...
  return (
    <div onClick={onClick} title={label} style={{ width:34, height:34, borderRadius:7, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:500, cursor:"pointer", border:`1.5px solid ${border}`, background:bg, color:clr, transition:"transform 0.15s", flexShrink:0 }}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) setDemandas(JSON.parse(saved));
    } catch {}
    setLoaded(true);
  }, []);
    ...
    <div style={{ fontFamily:"Amplesoft,Nunito,Segoe UI,sans-serif", background:"#F2F4F7", minHeight:"100vh", color:"#0B1F3F", fontSize:15 }}>
      ...
      <div style={{ background:"#0B1F3F", padding:"0 20px", display:"flex", alignItems:"center", justifyContent:"space-between", height:60, position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ color:"white", fontSize:17, fontWeight:500 }}>Compras <span style={{ color:"#FF6B6B", fontWeight:400 }}>2026</span></div>
        </div>
              ...
              return (
                <div key={d.id} style={{ background:"white", borderRadius:12, padding:18, border:`1px solid ${over?"rgba(227,30,36,0.25)":"#E8EBF0"}`, borderLeft:over?"4px solid #E31E24":"4px solid transparent" }}>
                  ...
                  <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                    {ETAPAS.map((e, ei) => <PStep key={e.key} done={d.etapas[e.key]} current={ei===(ea===-1?ETAPAS.length:ea)} label={e.short} onClick={() => toggle(d.id, e.key)} />)}
                    <div style={{ marginLeft:8, flex:1 }}>
                      <div style={{ height:6, background:"#E8EBF0", borderRadius:3, overflow:"hidden" }}>
                        <div style={{ width:`${pct}%`, height:"100%", background:pct===100?"#0FAA6E":pct>=40?"#E5A100":"#E31E24", borderRadius:3, transition:"width 0.3s" }} />
                      </div>
                      <div style={{ fontSize:13, color:"#7A869A", marginTop:3, fontWeight:400 }}>{pct}%</div>
                    </div>
                  </div>
