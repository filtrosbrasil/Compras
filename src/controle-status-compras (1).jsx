            ...
            {filtered.map(d => {
              ...
              return (
                <div key={d.id} style={{ background:"white", borderRadius:12, padding:18, border:`1px solid ${over?"rgba(227,30,36,0.25)":"#E8EBF0"}`, borderLeft:over?"4px solid #E31E24":"4px solid transparent" }}>
                  ...
                  {/* Row 3b: Dates */}
                  {/* Row 3b: Dates - always visible, editable */}
                  <div style={{ display:"flex", gap:16, marginBottom:12, fontSize:13, flexWrap:"wrap", alignItems:"center" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      ...
                      <input type="date" value={d.prazo||""} onChange={e => save(demandas.map(x => x.id===d.id?{...x,prazo:e.target.value}:x))}
                        style={{ border:"1px solid #E8EBF0", borderRadius:6, padding:"4px 8px", fontSize:13, fontFamily:"inherit", color:over?"#E31E24":soon?"#E07800":"#0B1F3F", background:"transparent", outline:"none", fontWeight:400 }} />
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                      ...
                      <input type="date" value={d.previsao_entrega||""} onChange={e => save(demandas.map(x => x.id===d.id?{...x,previsao_entrega:e.target.value}:x))}
                        style={{ border:"1px solid #E8EBF0", borderRadius:6, padding:"4px 8px", fontSize:13, fontFamily:"inherit", color:"#0B1F3F", background:"transparent", outline:"none", fontWeight:400 }} />
                    </div>
                  </div>
